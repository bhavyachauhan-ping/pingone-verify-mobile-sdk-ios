...
extension AppDelegate {
    private func handleDeepLink(_ launchOptions: [UIApplication.LaunchOptionsKey: Any]?) {
        if let userActDic = launchOptions?[UIApplication.LaunchOptionsKey.userActivityDictionary] as? [String: Any],
           let userActivity  = userActDic["UIApplicationLaunchOptionsUserActivityKey"] as? NSUserActivity {
            guard let appOpenUrl = userActivity.webpageURL, UIApplication.shared.canOpenURL(appOpenUrl) else {
                return
            }
            logattention("AppOpenUrl: \(appOpenUrl)")
            self.appOpenUrl = appOpenUrl.absoluteString
